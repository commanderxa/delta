...
mod tests {
    ...
    fn matmul_2d() {
        let a = delta::tensor(&[0., 1., 2., 3., 4., 5.], &[2, 3]).cuda();
        let b: Tensor = delta::tensor(&[6., 7., 8., 9., 10., 11.], &[3, 2]).cuda();
        let c = delta::tensor(&[28., 31., 100., 112.], &[2, 2]).cuda();
    ...
    /// Matrix multiplication
    fn matmul_2d_panic() {
        let a: Tensor = delta::tensor(&[6., 7., 8., 9., 10., 11.], &[3, 2]).cuda();
        let b = delta::tensor(&[28., 31., 100., 112.], &[2, 2]).cuda();
        linalg::matmul(b, a);
    }
    ...
    fn matmul_batched() {
        ...
        .cuda();
        let b = delta::tensor(&[9., 5., 3., 2., 6., 9., 5., 3., 2., 6.0], &[5, 2]).cuda();
