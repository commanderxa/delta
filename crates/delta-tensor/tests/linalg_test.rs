#[cfg(test)]
mod tests {
    use delta::{Tensor, get_default_dtype, linalg};

    #[test]
    /// Matrix multiplication
    fn matmul_2d() {
        let a = delta::tensor(&[0., 1., 2., 3., 4., 5.], &[2, 3], delta::cpu);
        let b: Tensor = delta::tensor(&[6., 7., 8., 9., 10., 11.], &[3, 2], delta::cpu);
        let c = delta::tensor(&[28., 31., 100., 112.], &[2, 2], delta::cpu);
    ...
    /// Matrix multiplication
    fn matmul_2d_panic() {
        let a: Tensor = delta::tensor(&[6., 7., 8., 9., 10., 11.], &[3, 2], delta::cpu);
        let b = delta::tensor(&[28., 31., 100., 112.], &[2, 2], delta::cpu);
        linalg::matmul(b, a);
    }
    ...
    #[test]
    fn cross_1d() {
        let a = delta::tensor(&[0.6, -20.5, 5.8], &[3], delta::cpu);
        let b = delta::tensor(&[10.2, -4.6, -34.], &[3], delta::cpu);
    ...
    #[test]
    fn cross_multidim() {
        let a = delta::ones(&[3, 2, 3, 2, 2, 3], get_default_dtype(), delta::cpu);
        let b = delta::tensor(
            &[
        ...
        let c = delta::linalg::cross(a.clone(), b.clone());
        let correct = vec![
    ...
    #[test]
    fn mul_scalar_2d() {
        let a = delta::arange(1., 11., 1.0, delta::float64, delta::cpu);
        let a = a.reshape(&[2, 5]);
        let b = 5 as i32;
        let c = a * b;
        assert_eq!(
    ...
    #[test]
    fn mul_scalar_3d() {
        let a = delta::arange(1., 13., 1.0, delta::float64, delta::cpu);
    ...
    #[test]
    fn mul_3d_and_1d() {
        let a = delta::arange(1., 9., 1.0, delta::float64, delta::cpu).reshape(&[2, 2, 2]);
        let b = delta::tensor(&[-2., 2.0], &[2], delta::cpu);
        let c = a * b;
    ...
    #[test]
    fn mul_3d_and_2d() {
        let a = delta::arange(1., 9., 1., delta::float64, delta::cpu).reshape(&[2, 2, 2]);
        let b = delta::tensor(&[-2., 3., 1., 4.], &[2, 2], delta::cpu);
        let c = a * b;
    ...
    #[test]
    fn mul_1d_and_1d() {
        let a = delta::tensor(&[10., 20., 30., 40., 50., 60.], &[6], delta::cpu);
        let b = delta::arange(1., 7., 1., delta::float64, delta::cpu);
        let c = a * b;
        assert_eq!(c.data::<f64>(), vec![10., 40., 90., 160., 250., 360.]);
    }

    #[test]
    fn div_scalar_2d() {
        let a = delta::arange(1., 61., 10., delta::float64, delta::cpu);
        let a = a.reshape(&[2, 3]);
        let b: i32 = 5;
        let c = a / b;
    ...
    #[test]
    fn div_scalar_3d() {
        let a = delta::arange(1., 13., 1., delta::float64, delta::cpu);
        let a = a.reshape(&[2, 2, 3]);
        let b: i32 = 5;
        let c = a / b;
        assert_eq!(
    ...
    fn div_3d_and_1d() {
        let a = delta::arange(1., 9., 1., delta::float64, delta::cpu).reshape(&[2, 2, 2]);
        let b = delta::tensor(&[-2., 2.], &[2], delta::cpu);
        let c = a / b;
        assert_eq!(
            c.data::<f64>(),
    ...
    fn div_3d_and_2d() {
        let a = delta::arange(1., 9., 1., delta::float64, delta::cpu).reshape(&[2, 2, 2]);
        let b = delta::tensor(&[-2., 3., 10., 4.], &[2, 2], delta::cpu);
        let c = a / b;
        assert_eq!(
            c.data::<f64>()
                .iter()
    ...
    fn div_1d_and_1d() {
        ...
        let b = delta::arange(1., 7., 1., delta::float64, delta::cpu);
        let c = a / b;
        assert_eq!(c.data::<f64>(), vec![10., 10., 10., 10., 10., 10.]);
    }
    ...
    #[test]
    fn add_1d_and_1d() {
        let a = delta::arange(0., 11., 1., delta::float64, delta::cpu);
        let b = delta::arange(0., 11., 1., delta::float64, delta::cpu);
        let c = a + b;
        assert_eq!(
            c.data::<f64>(),
    ...
    fn add_3d_and_3d() {
        let a = delta::arange(1., 9., 1.0, delta::float64, delta::cpu).reshape(&[2, 2, 2]);
        let b = delta::arange(8., 0., -1.0, delta::float64, delta::cpu).reshape(&[2, 2, 2]);
        let c = a + b;
    ...
    fn add_3d_and_2d() {
        let a = delta::arange(1., 9., 1.0, delta::float64, delta::cpu).reshape(&[2, 2, 2]);
        let b = delta::tensor(&[-2., 3., 10., 4.0], &[2, 2], delta::cpu);
        let c = a + b;
